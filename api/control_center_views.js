...
const titleOf = (row, fallback = 'Untitled') => row?.name || row?.full_name || row?.applicant_name || row?.tester_name || row?.display_name || row?.organization || row?.title || row?.email || fallback;
...
function renderApplications(rows){
  const actionable = rows.filter(row => ['new','hold','pending','reviewing'].includes(low(row.status || 'new')));
  const decided = rows.filter(row => ['accepted','declined','rejected'].includes(low(row.status))).slice(0,12);
  const intake = actionable.length ? `<div class="pm-table-wrap"><table class="pm-table"><thead><tr><th>Applicant</th><th>Role</th><th>Status</th><th>Submitted</th><th class="right">Action</th></tr></thead><tbody>${actionable.map(row=>`<tr data-prt-detail="application" data-prt-id="${row.id}"><td data-label="Applicant"><strong>${esc(titleOf(row,'Applicant'))}</strong><small>${esc(row.email || row.discord || row.iracing_name || '—')}</small></td><td data-label="Role">${esc(row.role || 'Driver')}</td><td data-label="Status">${statusBadge(row.status || 'new')}</td><td data-label="Submitted">${esc(age(row.created_at || row.submitted_at))}</td><td data-label="Action" class="right"><div class="pm-row-actions"><button class="pm-button pm-button-ghost" data-app-status="hold" data-id="${row.id}">Hold</button><button class="pm-button pm-button-ghost" data-app-status="declined" data-id="${row.id}">Decline</button><button class="pm-button pm-button-primary" data-app-status="accepted" data-id="${row.id}">Accept + onboard</button></div></td></tr>`).join('')}</tbody></table></div>` : empty('No applications need review right now.');
  const history = decided.length ? `<div style="margin-top:18px">${panel('Recent Decisions','Application History',`<div class="pm-table-wrap"><table class="pm-table"><thead><tr><th>Applicant</th><th>Role</th><th>Status</th><th>Submitted</th></tr></thead><tbody>${decided.map(row=>`<tr data-prt-detail="application" data-prt-id="${row.id}"><td data-label="Applicant"><strong>${esc(titleOf(row,'Applicant'))}</strong><small>${esc(row.email || row.discord || row.iracing_name || '—')}</small></td><td data-label="Role">${esc(row.role || 'Driver')}</td><td data-label="Status">${statusBadge(row.status)}</td><td data-label="Submitted">${esc(age(row.created_at || row.submitted_at))}</td></tr>`).join('')}</tbody></table></div>`)}</div>` : '';
  return panel('Tester Applications','PRT Intake',intake,`<span class="pm-badge ${actionable.length ? 'warn':'good'}">${n(actionable.length)} need review</span>`) + history;
}
function testerDiscord(row){
  const handle=String(row.discord_username || row.discord || '').trim();
  const display=String(row.discord_display_name || '').trim();
  const meta=[];
  if(display && low(display)!==low(handle)) meta.push(display);
  if(row.discord_connected) meta.push('Connected');
  else if(handle) meta.push('Provided');
  return {handle:handle || 'Not provided',meta:meta.join(' · ') || '—'};
}
function renderTesters(rows){return panel('Tester Program','Early Access',rows.length?`<div class="pm-table-wrap"><table class="pm-table"><thead><tr><th>Tester</th><th>Discord</th><th>iRacing / org</th><th>Invite</th><th>Tester state</th><th>Device</th><th>Last activity</th></tr></thead><tbody>${rows.map(row=>{const discord=testerDiscord(row);return `<tr data-prt-detail="tester" data-prt-id="${row.id}"><td data-label="Tester"><strong>${esc(titleOf(row,'Tester'))}</strong><small>${esc(row.email || row.hub_email || '—')}</small></td><td data-label="Discord"><strong>${esc(discord.handle)}</strong><small>${esc(discord.meta)}</small></td><td data-label="iRacing / org">${esc(row.iracing_name || '—')}</td><td data-label="Invite">${statusBadge(row.status)}</td><td data-label="Tester state">${statusBadge(row.tester_status)}</td><td data-label="Device">${esc(row.device_id || row.bound_device_id || 'Not bound')}</td><td data-label="Last activity">${esc(age(row.last_seen_at || row.redeemed_at || row.updated_at))}</td></tr>`}).join('')}</tbody></table></div>`:empty('No tester invites found.'))}
...
async function renderInsights(root,ctx){
  ...
  const recommendationRows=recommendations.length
    ? `<div class="pm-row-list">${recommendations.map(item=>`<div class="pm-row"><div class="pm-row-main"><div class="pm-row-meta">${statusBadge(item.priority||'info')}<span class="pm-badge orange">${esc(item.type||'signal')}</span></div><strong>${esc(item.title||'Recommendation')}</strong><p>${esc(item.reason||'')}</p><p><b>Do next:</b> ${esc(item.action||'')}</p></div></div>`).join('')}</div>`
    : empty('No recommendation rules are firing right now.');
  const commerceHeadline=commerce.orders
  ...
  root.innerHTML=`
    ...
    <div class="pm-grid pm-grid-2 pm-hq-lower">
      ${panel('Commerce Intelligence','Shopify Direct',details([
        ...
        ['Top product revenue',topProduct.title?'$'+Number(topProduct.revenue||0).toFixed(2):'—'],
        ['Units sold',n(topProduct.quantity)],
        ['Orders in window',n(commerce.orders)],
        ['Average order','$'+Number(commerce.average_order_value||0).toFixed(2)]
      ]))}
    ...
    <div class="pm-grid pm-grid-2 pm-hq-lower">
      ...
      ${panel('Paid Media','Meta Ads Direct',details([
        ...
        ['Spend','$'+Number(metaAds.summary?.spend||0).toFixed(2)],
        ...
        ['CPC',metaAds.summary?.cpc?'$'+Number(metaAds.summary.cpc).toFixed(2):'—']
      ]))}
    </div>
    <div class="pm-grid pm-grid-2 pm-hq-lower">
