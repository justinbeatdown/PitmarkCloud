...
def _require_admin(request: Request, admin_key: str | None):
...
def early_access_admin(
    ...
    _require_admin(request, x_pitmark_admin_key)
    return HTMLResponse(ADMIN_HTML, headers={'Cache-Control': 'no-store'})


@router.get('/control/early-access.css', include_in_schema=False)
def early_access_css(
    request: Request,
    x_pitmark_admin_key: str | None = Header(default=None),
):
    _require_admin(request, x_pitmark_admin_key)
...
@router.get('/control/early-access.js', include_in_schema=False)
def early_access_js(
    request: Request,
    x_pitmark_admin_key: str | None = Header(default=None),
):
...
<link rel="stylesheet" href="/control/early-access.css?v=0219">
<script src="/control/early-access.js?v=0219" defer></script>
</head>
...
async function load() {
  try {
    const d = await api('/api/entitlements/admin/early-access');
    const rows = d.items || [];
    if (!rows.length) {
      $('tablewrap').innerHTML = '<div class="empty">No tester invites yet.</div>';
      return;
    }
    let h = '<table><thead><tr><th>TESTER</th><th>CODE</th><th>STATUS</th><th>DEVICE</th><th>CREATED</th><th></th></tr></thead><tbody>';
    for (const x of rows) {
      h += `<tr><td><strong>${esc(x.applicant_name)}</strong><br><span class="muted">${esc(x.email)}</span></td><td><span class="hint">${esc(x.code_hint)}</span></td><td><span class="status ${esc(x.status)}">${esc(x.status)}</span><br><span class="muted">${esc(x.tester_status)}</span></td><td><span class="hint">${esc(x.bound_device_id || '—')}</span></td><td>${esc((x.created_at || '').replace('T', ' ').slice(0, 16))}</td><td>${x.status !== 'revoked' ? `<button type="button" class="danger" data-revoke="${x.id}">REVOKE</button>` : ''}</td></tr>`;
    }
    h += '</tbody></table>';
    $('tablewrap').innerHTML = h;
    document.querySelectorAll('[data-revoke]').forEach(b => b.addEventListener('click', async () => {
      if (!confirm('Revoke this Early Access invite?')) return;
      try {
        await api('/api/entitlements/admin/early-access/' + b.dataset.revoke + '/revoke', {method: 'POST'});
        await load();
      } catch (e) {
        alert(e.message);
      }
...
function init() {
  ...
  $('copyCode').addEventListener('click', () => copy($('code').textContent, $('copyCode')));
  $('copyMessage').addEventListener('click', () => copy(lastMessage, $('copyMessage')));
