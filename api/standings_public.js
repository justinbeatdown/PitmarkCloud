...
const pageView=routePath==='/standings'||routePath.endsWith('/standings')
  ...
  :routePath.endsWith('/schedules')
    ...
    :routePath.endsWith('/live')
      ?'live'
      :'hub';
const PREF_KEY='pitmark-race-center-v6';
const CACHE_KEY='pitmark-race-center-v6-feed';
const readPrefs=()=>{
  try{
...
function configurePage(){
  ...
  const config={
    hub:{
      ...
      pageTitle:'Pitmark Race Center V6 — Racing Hub'
    },
    standings:{
...
async function load(){
  try{
    ...
    const response=await fetch('/api/public/standings?v=race-center-v6-20260922',{
