...
function showYouTubeConnectPanel(){
  ...
  panel.className='native-modal-backdrop';
  panel.innerHTML='<div class="native-modal"><span class="section-label">YouTube · Read Only</span><h2>Connect the Pitmark YouTube channel</h2><p>If Google shows <code>403 org_internal</code>, the OAuth app audience is still set to <strong>Internal</strong>. That cannot be bypassed in code. Open the Google Auth audience page for Pitmark Cloud, switch the audience to <strong>External</strong>, add the Google account that owns the Pitmark YouTube channel as a test user if Google asks, then retry this button.</p><div class="setup-actions"><a class="inline-link" href="https://console.cloud.google.com/auth/audience?project=542087459198" target="_blank" rel="noopener noreferrer">Open Google Auth Audience</a></div><p>After Google authorizes read-only YouTube access, it will finish on a localhost page that may say it cannot connect; copy the <strong>entire URL</strong> from the address bar, return here, and paste it below.</p><label for="youtube-callback-url">YouTube callback URL</label><textarea id="youtube-callback-url" placeholder="http://127.0.0.1:8765/?state=...&code=..."></textarea><div class="native-modal-actions"><button type="button" id="youtube-cancel">Cancel</button><button type="button" class="native-action" id="youtube-complete">Complete connection</button></div><small id="youtube-connect-status"></small></div>';
  document.body.appendChild(panel);
