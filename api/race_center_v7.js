(function(){
  ...
  async function renderMyRacing(){
    ...
    try{
      ...
      blocks.push('<section class="v7-brief-section"><span class="eyebrow">RECENT RESULTS</span><h3>What just happened</h3><div class="v7-brief-row">'+((data.recent_results||[]).map(e=>{
        const winner=(e.results||[]).find(row=>String(row.position||'')==='1')||(e.results||[])[0]||{};
        return briefCard('RESULT',e.name,[e.series_name,e.venue,winner.name||winner.driver?('Winner: '+(winner.name||winner.driver)):''].filter(Boolean).join(' · '),'/race-center/event/'+encodeURIComponent(e.key));
      }).join('')||'<p>No newly published results among your followed racing.</p>')+'</div></section>');
      ...
      blocks.push('<section class="v7-brief-section"><span class="eyebrow">PITMARK COVERAGE</span><h3>Stories about your racing</h3><div class="v7-brief-row">'+((data.coverage||[]).map(article=>briefCard('COVERAGE',article.title,article.summary||'Pitmark coverage connected to something you follow.',article.url)).join('')||'<p>No new Pitmark coverage is attached to your followed racing.</p>')+'</div></section>');
